...
- **Blocages assignation** (`lib/slot-blocking.ts`) : jamais de médecin en congés hors ligne Congés ; ½-off Matin ⇒ pas d’activité matin ; ½-off Apm ⇒ pas d’activité apm ; 1 tâche / créneau matin|apm (sauf **ATL+Coro** et **ETT salle1+salle2**) ; **pas** de cumul Cs PSS+Tessée ; Garde Matin/Midi exclusives ; **LFB/CDL** interdits jour de garde + lendemain. **Doublon** : Cs = 2× dans la **même** case → `B²` (2ᵉ clic) ; ETT = présent sur **salle 1 et 2** → `S²`. UI : `canAssignDoctor(..., { schedule, day })` + `applySlotBlockingStrips`.
- **Astreintes ATL** :
