...
- **« Générer » (une seule passe) :** `generateGuardsViaAPI` calcule `historical_patterns` via `buildHistoricalPatternsPayload` (`lib/pattern-analysis.ts`, **allowlist** Cs/ETT/EE/Stress uniquement — `isSolverHistoricalRowKey` ; ½-off / Visite / hors site exclus pour éviter les warnings « row_key non reconnu »). Envoyé dans `/generate-week`. Le solveur OR-Tools doit consommer ce champ pour la fidélité historique — **pas de fill client post-génération**. **INFEASIBLE classique** si ASTREINTE+CORO exclusifs sur le même créneau alors que ATL←Coro force les deux : le solveur excepte cette paire (`guard-api/solver.py` §3). Rythmo forcé = P mardi matin+apm, U mercredi **apm**, A lundi/jeudi **apm** (comme `fixed-assignments`). Déployer `guard-api/` sur Render après changement solveur.
