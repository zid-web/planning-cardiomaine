...
- **Vercel / v0 URLs:** Some aliases use Deployment Protection SSO (302 → `vercel.com/sso-api`); others (e.g. older `v0-recreate-attached-p2ci9kbnq-…`) may be public. Prefer a stable public domain for cron. GitHub “Vercel: Deployment failed” after PR #37 was correlated with `vercel.json` cron `*/5 * * * *` on Hobby — keep Hobby-safe daily cron only; use an external 5‑min ping for Render keep-alive. Always check deploy SHA vs `main` HEAD before assuming `/api/ping-solver` is live.
