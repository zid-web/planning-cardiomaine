...
- **« Générer » / vacations matin–soir (Coro) :** le frontend mappe `CORO`/`VACANCES`/`PRE_OP`/`RYTHMO` via `resolveRowKey` et fusionne avec `mergeSolverWeekIntoExisting` (préserve Cs/ETT déjà saisis). Le solveur Render doit forcer **CORO matin + am** Lundi–Vendredi (`guard-api/solver.py` §5ter) — sinon l’équité laisse Coro vide. Après changement solver, déployer `guard-api/` sur Render (le bot ne peut pas push `guard-api-cardiomaine`).
