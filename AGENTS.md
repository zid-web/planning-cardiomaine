...
- Règles fixes centralisées dans `lib/fixed-assignments.ts` (`applyFixedClinicalAssignments`) : **IRM = S** (Lundi matin + Vendredi après-midi), **FV** Garde Nuit Lundi + Coro Jeudi apm, **DAAS** = `Apm - EE2` Lundi, **Rythmo** = P mardi (matin+apm), U mercredi apm, A lundi/jeudi apm, **Visite** = rotation `U → A → B`. **Si le titulaire est en congés, la contrainte saute** : case libre, pas de réinjection, couverture manuelle (ex. **P** mercredi si U absent) conservée. `getAllVacations` doit échouer proprement (pas `[]` silencieux) pour ne pas écraser Congés/Rythmo.
...
- Règle (`lib/half-day-off.ts`) : après Garde Nuit → **apm du lendemain** (sauf **samedi**). Si off **habituel** apm → **matin**. **À chaque modification** de Garde Nuit : reconstruit ½ off matin+apm du lendemain. Dimanche → lundi semaine suivante. **½ off apm fixes** : Mardi `S` ; Mercredi `M,W,G,Z,H,B` ; Jeudi `U,P` ; Vendredi `O,A,K,R,T`. Miroir solveur : `guard-api/solver.py` (`half_days_off` / `habitual_afternoon_off`).
...
- **Backend Render = dépôt séparé** `github.com/zid-web/guard-api-cardiomaine` (service `guard-api-cardiomaine.onrender.com`). Le dossier `guard-api/` dans **ce** repo (`planning-cardiomaine`) n’est qu’un **miroir / source à copier** — un commit ici **ne déploie pas** Render. Le bot Cursor **n’a pas le droit push** sur `guard-api-cardiomaine` (403) : un humain doit merger ou coller `guard-api/solver.py` + `rules_config.json` (ou appliquer `guard-api/patches/fix-generate-historical-patterns.patch`) sur la branche déployée Render.
- **« Générer » (une seule passe) :** `generateGuardsViaAPI` calcule `historical_patterns` via `buildHistoricalPatternsPayload` (`lib/pattern-analysis.ts`, **allowlist** Cs/ETT/EE/Stress uniquement — `isSolverHistoricalRowKey` ; ½-off / Visite / hors site exclus). Le solveur Render (`guard-api-cardiomaine`) ignore aussi ces clés silencieusement. Rythmo forcé = P mardi matin+apm, U mercredi **apm**, A lundi/jeudi **apm** (`rythmo_slots` dans `rules_config.json`).
- **PDF « JSON malformé »** : root cause is Claude Vision truncation/`json.loads` in **`guard-api-cardiomaine`** (`pdf_upload.py`), not the Next.js proxy. Fixed sources to copy/deploy live in `guard-api/` (+ patches). Cursor bot cannot push that repo — a human must merge onto Render’s deploy branch.
