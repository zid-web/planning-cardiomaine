...
- **« Générer » (une seule passe) :** `generateGuardsViaAPI` calcule `historical_patterns` via `buildHistoricalPatternsPayload` (`lib/pattern-analysis.ts`, exclusions = `FREQUENCY_EXCLUDED_ROW_KEYS`) et l’envoie dans le payload `/generate-week`. Le solveur OR-Tools doit consommer ce champ pour Cs/ETT/EE/hors site (fidélité historique) — **pas de fill client post-génération** (ex-Pré-remplir / Historique+ retiré). Réception = même format `assignments` ; étendre `ACTIVITY_TO_ROW` / `GENERATOR_PROPOSAL_ROW_KEYS` quand les codes activité Cs/ETT/EE sont figés côté Render. Déployer `guard-api/` sur Render.
