...
- Admin change-requests UI lives at **`/protected/admin/requests`** (`app/protected/admin/requests/page.tsx`): filters (status / date range / requester / requested doctor), pagination (20/page, `count: 'exact'` + `range`), full history (all statuses), detail modal with reject comment. Approve/reject stays in `app/actions/change-request-actions.ts`. The old path `/protected/change-requests` is a thin `redirect()` only — do not reintroduce a full page there.
