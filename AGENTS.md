...
- **Blocages assignation** (`lib/slot-blocking.ts`) : jamais de médecin en congés hors ligne Congés ; ½-off Matin ⇒ pas d’activité matin ; ½-off Apm ⇒ pas d’activité apm ; 1 tâche / créneau matin|apm (sauf **ATL+Coro** et **doublon Cs** PSS+Tessée / **ETT** salle1+2 → affichage `²`) ; Garde Matin/Midi exclusives sur leur créneau ; **LFB/CDL** interdits le jour d’une garde et le lendemain. UI : `canAssignDoctor(..., { schedule, day })` + strip via `applySlotBlockingStrips` dans `applyStructuralConstraints`.
- **Astreintes ATL** :
