...
- **Astreintes ATL** :
  - **Pool coronarographistes uniquement** : **M, O, W, FV, CH** (Matin / Midi / Soir). **Pas** R, V, T, G ni autres permanents. Front : `astreinte_allowed` dans `rules_override` + filtre `applyAtlCoronarographisteEligibility` + `can_be_assigned_to_astreinte`. Backend : appliquer `patches/guard-api-astreinte-coronarographistes.patch` sur `guard-api-cardiomaine` (sinon le solveur crée encore des vars ASTREINTE pour tout `PERMANENT`).
- **ATL Matin/Midi Lun–Ven = Coro** : c’est **la même affectation** (même médecin). `applyAtlFollowsCoroConstraints` unifie les deux lignes ; la saisie UI (`patchSelectedCell` / validation) écrit les deux. Cumul autorisé dans `slot-blocking` car c’est un doublon volontaire de ligne, pas deux tâches.
