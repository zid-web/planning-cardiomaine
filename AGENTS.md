...
- **« Générer » (une seule passe) :** `generateGuardsViaAPI` calcule `historical_patterns` via `buildHistoricalPatternsPayload` (`lib/pattern-analysis.ts`, **allowlist** Cs/ETT/EE/Stress — `isSolverHistoricalRowKey`). Le solveur **`guard-api-cardiomaine`** applique la même allowlist + Rythmo précis (`rythmo_slots`, vendredi P/U). **Pas de fill client post-génération.**
