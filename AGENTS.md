...
- **Vercel / v0 URLs:** Deployment Protection SSO is often ON for `v0-recreate-attached-*-zids-projects-22b662f4.vercel.app` (302 → `vercel.com/sso-api`). External cron keep-alive and unauthenticated probes need a **public production domain** (or a Deployment Protection bypass token). GitHub “Vercel: Deployment failed” on `main` can coexist with older successful Production aliases — always check the deploy SHA vs `main` HEAD before assuming `/api/ping-solver` is live.
