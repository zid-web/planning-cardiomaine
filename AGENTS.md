...
- **Blocages assignation** (`lib/slot-blocking.ts`) : jamais de médecin en congés hors ligne Congés ; ½-off Matin ⇒ pas d’activité matin ; ½-off Apm ⇒ pas d’activité apm ; 1 tâche / créneau matin|apm (sauf **ATL+Coro**) ; **pas** de cumul Cs PSS+Tessée ni ETT1+ETT2 le même créneau ; Garde Matin/Midi exclusives ; **LFB/CDL** interdits jour de garde + lendemain. **Doublon** : 2× le même médecin dans la **même** case Cs ou ETT seulement → badge `B²` (2ᵉ clic). UI : `canAssignDoctor(..., { schedule, day })` + `applySlotBlockingStrips`.
- **Astreintes ATL** :
