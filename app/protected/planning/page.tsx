...
console.log('🔍 NEW PAGE LOADED - /protected/planning');
...
export default function PlanningPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log('[v0] Loading data for week:', weekKey);
        ...
        if (scheduleData) {
          console.log('[v0] Schedule loaded from DB');
          setSchedule(scheduleData.schedule_data);
        } else {
          console.log('[v0] No schedule found, using empty');
        ...
        const { data: vacationsData } = await supabase
          ...
          .select("*");
        console.log('[v0] Vacations loaded:', vacationsData?.length || 0);
  ...
  const saveSchedule = useCallback(async (newSchedule: ScheduleData) => {
    try {
      console.log('[v0] Saving schedule for week:', weekKey);
  ...
  const prevWeek = () => {
    ...
    console.log('[v0] Previous week:', d);
    setCurrentDate(d);
  };
  const nextWeek = () => {
    const d = new Date(currentDate);
    d.setDate(d.getDate() + 7);
    console.log('[v0] Next week:', d);
    setCurrentDate(d);
  };
  const goToToday = () => {
    console.log('[v0] Going to today');
  ...
  const handleScheduleUpdate = useCallback((newSchedule: any) => {
    try {
      console.log('[v0] Updating schedule from voice/PDF');
        ...
        {isAdmin && (
          <div className="bg-white p-4 rounded-xl shadow-sm border">
            <VoiceAndUploadPanel
              ...
              onPdfParsed={(data) => {
                console.log("[v0] PDF data extracted:", data);
              }}
            />
          </div>
        )}
