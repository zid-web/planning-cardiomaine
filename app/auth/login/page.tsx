...
// ─── Bannière « Ouvrir dans Safari » ──────────────────────────────────────────
// Sur iOS, Chrome/Firefox/etc. utilisent bien le moteur WebKit, mais SEUL
// Safari expose « Sur l'écran d'accueil » comme véritable app installable
// (icône, mode plein écran, service worker). Depuis Chrome iOS, « Ajouter à
// l'écran d'accueil » ne crée qu'un simple favori qui rouvre Chrome — d'où
// l'impression que « l'app n'est pas installable ». On détecte ce cas et on
// propose de rouvrir directement dans Safari via le schéma d'URL `x-safari-`,
// qui fonctionne depuis n'importe quel navigateur iOS pour forcer l'ouverture
// dans Safari sans que l'utilisateur ait à copier-coller le lien lui-même.
function OpenInSafariBanner({ target }: { target: InstallTarget }) {
  const [dismissed, setDismissed] = useState(false)

  if (target !== "ios-other" || dismissed) return null
  ...
  return (
    ...
    >
      <span>
        Sur iPhone/iPad, l&apos;installation de l&apos;app n&apos;est possible que depuis{" "}
        <strong>Safari</strong>. Ce navigateur ne le permet pas.
      </span>
      <button
        type="button"
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Sur iOS, le clavier (barre QuickType, correction automatique,
      // remplissage automatique iCloud Keychain) ajoute très souvent un
      // espace en fin de champ dès qu'une suggestion est tapée. Cet espace
      // invisible fait échouer signInWithPassword avec "Invalid login
      // credentials" alors que l'utilisateur a bien saisi les bons
      // identifiants — c'est la cause n°1 des connexions refusées sur
      // iPhone. L'email est en plus normalisé en minuscules : iOS
      // capitalise parfois la première lettre malgré `type="email"`.
